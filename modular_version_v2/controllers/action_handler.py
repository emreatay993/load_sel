        ...
        if data_domain == 'TIME':
            ...
            if tab.section_checkbox.isChecked():
                try:
                    ...
                    if t_min < t_max:
                        df_processed = df_processed[(df_processed['TIME'] >= t_min) & (df_processed['TIME'] <= t_max)].copy()
                    else:
                        QMessageBox.warning(self.main_window, "Invalid Range", "Min Time must be less than Max Time.")
                except ValueError:
                    QMessageBox.warning(self.main_window, "Invalid Input", "Please enter valid numeric values for Min and Max Time.")

            if tab.tukey_checkbox.isChecked():
                if len(df_processed) > 1:
                    alpha = tab.tukey_alpha_spin.value()
