...
def insert_phase_columns(df):
    # Create a list to store the transformed columns temporarily
    transformed_columns = []

    # Iterate over the original columns to create new columns and phase information
    ...
    # Concatenate all columns horizontally to form the final DataFrame
...
class PlotlyGraphs(QWidget):
    ...
    def extract_frequency_data(self):
        ...
        try:
            ...
            # Filter DataFrame for the selected frequency
            filtered_df = self.df[self.df['FREQ'] == freq]

            # Compile pattern to match the selected side and ensure the 'FREQ' column is included
            side_pattern = re.compile(re.escape(selected_side))
            columns = ['FREQ']  # Start with the frequency column
            ...
            # Convert specific columns by multiplying by 1000 (exclude 'FREQ' and any 'Phase_' columns)
            ...
            QMessageBox.information(self, "Extraction Complete", f"Data has been extracted and converted. Original data saved to {original_file_path}. Converted data saved to {converted_file_path}.")
        except Exception as e:
