...
class PlotlyGraphs(QWidget):
    ...
    def setupCompareTab(self, tab):
        ...
        splitter.addWidget(self.compare_regular_plot)
        splitter.addWidget(self.compare_phase_plot)
        splitter.setSizes([self.height() // 2, self.height() // 2])
    ...
    def update_compare_plots(self):
        ...
        if selected_column and self.df_compare is not None:
            ...
            fig_reg = go.Figure()
            fig_reg.add_trace(go.Scatter(x=x_data, y=self.df[selected_column], mode='lines', name=f'Original {selected_column}',
                                         hovertemplate=custom_hover))
            fig_reg.add_trace(go.Scatter(x=x_data_compare, y=self.df_compare[selected_column], mode='lines', name=f'Compare {selected_column}',
                                         hovertemplate=custom_hover))
            ...
            fig_phase = go.Figure()
            fig_phase.add_trace(go.Scatter(x=x_data, y=self.df[phase_column], mode='lines', name=f'Original {phase_column}',
                                           hovertemplate=custom_hover))
            fig_phase.add_trace(go.Scatter(x=x_data_compare, y=self.df_compare[phase_column], mode='lines', name=f'Compare {phase_column}',
                                           hovertemplate=custom_hover))
